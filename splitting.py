...
import numba
from .histogram import _build_histogram_naive
...
from .histogram import _build_histogram_root_no_hessian_naive
...
def split_indices(context, split_info, sample_indices):
    ...
    X_binned = context.X_binned.T[split_info.feature_idx].T
    ...
    for thread_idx in prange(n_threads):
        ...
        for i in range(start, stop):
            sample_idt, sample_idd = sample_indices[i]
            if X_binned[sample_idt, sample_idd] <= split_info.bin_idx:
                left_indices_buffer[start + left_count] = (sample_idt, sample_idd)
                left_count += 1
            else:
                right_indices_buffer[start + right_count] = (sample_idt, sample_idd)
                right_count += 1
...
def find_node_split(context, sample_indices):
    ...
    if sample_indices.shape[0] != ctx.gradients.shape[0] * n_design :
        n_threads = numba.config.NUMBA_DEFAULT_NUM_THREADS
        ...
        if ctx.constant_hessian:
            ...
            #                   starts[thread_idx] + sizes[thread_idx]):
            #        print(i)
            ordered_gradients = ctx.gradients
        else:
            ...
            for thread_idx in prange(n_threads):
                for i in range(starts[thread_idx],
                               starts[thread_idx] + sizes[thread_idx]):
                    ordered_gradients[i] = ctx.gradients[i]
                    #ordered_hessians[i] = ctx.hessians[sample_indices[i]]

    ctx.sum_gradients = np.sum([np.dot(ctx.ordered_gradients[i], ctx.yd[j]) \
...
def _find_histogram_split(context, feature_idx, sample_indices):
    ...
    """
    #print('find_histo_split')
    n_samples = sample_indices.shape[0]
    X_binned = context.X_binned[:,:,feature_idx]
    yd = context.yd
    assert(X_binned.shape[1]==yd.shape[0])
    root_node = X_binned.shape[0]*X_binned.shape[1] == n_samples
    #print('root_node', root_node)
    ordered_gradients = context.ordered_gradients
    ordered_hessians = context.ordered_hessians

    if root_node:
        #print('root_here')
        if context.constant_hessian:
            #print('yes')
            histogram = _build_histogram_root_no_hessian_naive(
                context.max_bins, X_binned, ordered_gradients, yd)
            #print('root histo', histogram)
        else:
            #won't be used
        ...
        #print('not root')
        if context.constant_hessian:
            histogram = _build_histogram_naive(
                context.max_bins, sample_indices, X_binned,
                ordered_gradients, yd)
        else:
            #won't be used
...
def _find_histogram_split_subtraction(context, feature_idx,
    ...
    Returns the best SplitInfo among all the possible bins of the feature.
    """
    #print('soustraction histo attention!!' )
    histogram = _subtract_histograms(
        context.max_bins,
        parent_histograms[feature_idx], sibling_histograms[feature_idx])
    sample_indices = histogram['sample_indices']
    #print('\r sample_indices', sample_indices)
    return _find_best_bin_to_split_helper(context, feature_idx, histogram,
                                         sample_indices, n_samples)
...
#              'n_samples_left': uint32},
#      fastmath=True)
def _find_best_bin_to_split_helper(context, feature_idx, histogram, sample_indices, 
                                    n_samples):
    """Find best bin to split on, and return the corresponding SplitInfo.

    Splits that w constraints (min_gain_to_split,
    ...
    for bin_idx in range(context.n_bins_per_feature[feature_idx]):
        ...
        #print('gradient_left', gradient_left)
        gradient_right = context.sum_gradients - gradient_left
        #print('gradient_right', gradient_right)
        if n_samples_left < context.min_samples_leaf:
            #print('hola', n_samples_left, context.min_samples_leaf)
            continue
        if n_samples_right < context.min_samples_leaf:
            #print('aieaieaie', n_samples_right , context.min_samples_leaf)
        ...
        if hessian_right < context.min_hessian_to_split:
            #print('oups',hessian_right, context.min_hessian_to_split)
            # won't get any better (hessians are > 0 since loss is convex)
            break
        n1, n2 = context.H.shape
        assert(n1==n2)
        #print('n1,n2', n1,n2)
        ...
        ydd = context.yd
        hessian_split[0,2:] = np.array([H_builder(sample_indices_left,
         n.sample_indices, ydd) for n in context.list_current_leaves])
        hessian_split[2:,0] = hessian_split[0,2:]
        hessian_split[1,2:] = np.array([H_builder(sample_indices_right,
         n.sample_indices, ydd) for n in context.list_current_leaves])
        hessian_split[2:,1] = hessian_split[1,2:]
        ...
        if gain > best_split.gain and gain > context.min_gain_to_split:
            ...
            best_split.gradient_split = gradient_split
            best_split.hessian_split = hessian_split
            ...
            best_split.sample_indices_left = sample_indices_left
            best_split.sample_indices_right = sample_indices_right
    #print('best_split', best_split.feature_idx, best_split.bin_idx)
