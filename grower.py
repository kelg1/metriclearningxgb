...
class TreeNode:
    ...
    def __init__(self, depth, sample_indices, gradients, sum_gradients,
                 hessians, yd, parent=None):
        self.depth = depth
...
class TreeGrower:
    ...
    def __init__(self, X_binned, gradients, hessians, yd, max_leaf_nodes=None,
        ...
        self.yd = yd
        self.n_design = len(yd)
        self.output_dim = np.reshape(yd, (-1, 1)).shape[1]
    ...
    def grow(self):
        ...
        while self.can_split_further():
            ...
            final_leaves = self.finalized_leaves
            g_, h_ = self._gather_temporal_leaves()
            #print('H', h_)
            #print('g', g_)
            ...
            ## SANS CONTRAINTES SUR w ##
            #values = - self.shrinkage * np.dot(np.linalg.inv(self.splitting_context.l2_regularization*np.eye(nl) + \
            #        self.splitting_context.H), self.splitting_context.G)
            #print(values)
            
            ## TEST AVEC CONTRAINTES SUR w ## 
            def obj(x):
    ...
    def _gather_temporal_leaves(self):
        current_leaves = self.splittable_nodes + self.finalized_leaves
    ...
    def _intilialize_root(self):
        ...
        if self.splitting_context.constant_hessian:
            #hessian = [self.splitting_context.hessians[0] * n_design] * n_samples
            # why don't use hessian = 1 ? #
            hessian = self.splitting_context.hessians[0]
        else:
        ...
        self.root = TreeNode(
            ...
            sum_gradients= np.sum([np.dot(gi, yj) for gi in self.splitting_context.gradients 
                for yj in self.yd]), 
            hessians=hessian, yd=self.yd
        )

        heappush(self.list_current_leaves, self.root)
        self.splitting_context.list_current_leaves = self.list_current_leaves
        if (self.max_leaf_nodes is not None and self.max_leaf_nodes == 1):
            #print('oups', 'initroot', self.max_leaf_nodes)
    ...
    def _compute_spittability(self, node, only_hist=False):
        ...
        if node.split_info is None and node.histograms is None:
            ...
            tic = time()
            if node.hist_subtraction and False:
                ...
                split_info, histograms = find_node_split(
                    self.splitting_context, node.sample_indices)
            #print('feature_idx', split_info.feature_idx)
            toc = time()
    ...
    def split_next(self):
        ...
        self.list_current_leaves = self.finalized_leaves + self.splittable_nodes
        n_leaf_nodes += 1

        left_child_node = TreeNode(depth=depth,
                                    sample_indices=sample_indices_left,
                                    gradients=self.splitting_context.gradients,
                                    sum_gradients=node.split_info.gradient_split[0], 
                                    hessians=self.splitting_context.hessians[0], yd=self.splitting_context.yd,
                                    parent=node
                                    )
        right_child_node = TreeNode(depth=depth,
                                    sample_indices=sample_indices_right,
                                    gradients=self.splitting_context.gradients,
                                    sum_gradients=node.split_info.gradient_split[1], 
                                    hessians=self.splitting_context.hessians[0], yd=self.splitting_context.yd,
                                    parent=node
                                    )
    ...
    def _finalize_leaf(self, node,):
        ...
        #print('whichnode', node)
        #nl = len(self.splitting_context.G)
        #print(nl)
        #print('final', nl)
        #indice_node = [1 if n==node else 0 for n in self.splittable_nodes + self.finalized_leaves]
        #print('bambimboum H', self.splitting_context.H.shape)
        #print('bambimboum G', np.shape(self.splitting_context.G))
        node.gain = -1 
        self.finalized_leaves.append(node)

        #print(len(self.finalized_leaves + self.splittable_nodes))
        #values = - np.dot(np.linalg.pinv(self.splitting_context.l2_regularization*np.eye(nl) + \
        #            self.splitting_context.H), self.splitting_context.G)
        #for i,n in enumerate(self.finalized_leaves + self.splittable_nodes):
        #    if n==node:
        #        n.value = values[i]
